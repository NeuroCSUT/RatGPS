function [err,decXY,trueXY,mlVal]=decode_crunchData_SpecicCells( dirFln, tet2use, dwnSampCell, varargin)
% NOTE: modified version to allow iteration over specific cells i.e.
% because we wanted to down sample
...
for nCV         =1:nCVstep
    ...
    for n           =1:nCells % is slower with parfor!
        ...
        rms(:,:,n)      =make_smooth_ratemap(binPos, binSpk, kernSz, kernType); %ratemaps
    ...
    %Store this for each fold of the cross validation - NB two versions
    %here. Comment one out as required. First is original ML version second
    %is full Bayes with history
        post            =decode_calcPost(spkCnt(dwnSampCell,:), rms(:,:,dwnSampCell), tBin); %3D mat [envSz1 x envSz2 x nTBin]
%     post            =decode_calcBayesPost(spkCnt(dwnSampCell,:), rms(:,:,dwnSampCell), dms, tBin); %3D mat [envSz1 x envSz2 x nTBin]
